...
func New(ctx context.Context, log logr.Logger, kubeClient ctrlclient.Client, store smv1alpha1.GenericStore, namespace string) (store.Client, error) {
	...
	log.Info("pppp")
	address := v.store.GetSpec().Vault.Server
	//cfg, err := v.newConfig(log)
	cfg := &vault.Config{
		Address:    address,
	...
	if err := v.setToken(ctx, client, log); err != nil {
		return nil, err
	}
...
func (v *Vault) newConfig(log logr.Logger) (*vault.Config, error) {
	cfg := vault.DefaultConfig()
	cfg.Address = v.store.GetSpec().Vault.Server

	log.Info(fmt.Sprintf("Address: %s", cfg.Address))

	certs := v.store.GetSpec().Vault.CABundle
	if len(certs) == 0 {
		log.Info(fmt.Sprintf("Return"))
		return cfg, nil
	}
...
func (v *Vault) setToken(ctx context.Context, client Client, log logr.Logger) error {
